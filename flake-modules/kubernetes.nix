topLevel@{ flake-parts-lib, lib, ... }: {
  ...
  config.flake.lib = {
    ...
    pathToKubernetesName = path: lib.trivial.pipe path [
      (builtins.split "/")
                          ...
                          config.base-package = pkgs.runCommand "${runtime.config._module.args.name}-push-image.txt"
                            {
                              ...
                              remoteImage =
                                "${
                                  ...
                                  kubernetes.config.imageRegistry.path
                                }/${
                            ...
                            ''
                              skopeo \
                                --insecure-policy \
                                copy \
                                --dest-registry-token "$destRegistryToken" \
                                "nix:$specification" \
                                "docker://$remoteImage" &&
                              printenv remoteImage > "$out"
                            '';
                        }
                      ];
                    };
                          ...
                          gcloud auth print-access-token |
                            ...
                            ${
                              lib.strings.escapeShellArgs [
                                ...
                                "oci://${kubernetes.config.imageRegistry.host}/${kubernetes.config.imageRegistry.path}"
                              ]
                            } &&
